    ...
    public partial class AddProduct : Window
    {
        ...
        private void ButtonLoadImage(object sender, RoutedEventArgs e)
        {
            ...
            if (openFile.ShowDialog() == true)
            {
                ...
                if (select.Width > 500 || select.Height > 500)
                {
                    MessageBox.Show("Размеры изображения имеют не верный формат");
                    return;
                }

                // Генерируем уникальное имя, чтобы файлы не перезаписать
                string extension = Path.GetExtension(openFile.SafeFileName);
                string uniqueFileName = $"{Guid.NewGuid()}{extension}";

                string destinationPath = Path.Combine(projPath, "Images", uniqueFileName);
